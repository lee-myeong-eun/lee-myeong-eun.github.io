...
```
    ...
    greeting1, 선생님이 말했다."항상 긍정적으로 생각하라"
...
```
    ...
    p,y,t,h,o,n, ,i,s, ,v,e,r,y, ,n,i,c,e
...
```
    ...
    이명은 님, 5년 후 나이는 29세이고, 키는 160.0cm입니다.
...
### remove는 리스트에서 첫 번째로 나오는 x를 삭제하는 함수
...
### 예제를 보면, 입력 인수를 받는 매개변수도 없고 return문도 없어 입력값,반환값 모두 없다.
...
### 1.  파이썬 함수의 반환값은 항상 하나이다.





### 2. 여러 값을 반환하는 것처럼 보일 경우, 실제로는 튜플 형태로 묶여 하나의 값으로 반환된다.
...
### 3. 튜플 반환값은 언패킹을 통해 각각의 변수로 나눠 받을 수 있다.
...
### 4. return문은 실행되는 즉시 함수가 종료되며, 이후 코드는 실행되지 X
...
### 5. return은 값을 반환하지 않고 함수를 종료하는 용도로도 사용된다.
...
### 매개변수 초깃값 미리 설정하기 (Default Parameter)





### 1. 함수 정의 시 매개변수에 기본값을 설정할 수 있다.

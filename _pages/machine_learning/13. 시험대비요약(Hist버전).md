---
layout: default
title: 13. 시험대비요약(Hist버전)
permalink: /assignments/machine-learning/exam-summary-hist/
---
...
===== Classification Report (Test Data) =====
           ...
           1       0.82      0.98      0.90       109

    accuracy                           0.97       900
   macro avg       0.91      0.98      0.94       900
...
모델 학습 에러 발생 시: 만약 HistGradientBoostingClassifier가 작동 안 하면 Cell 3 상단에서 HistGradientBoostingClassifier를 GradientBoostingClassifier로 바꾸고 class_weight 옵션 대신 sample_weight를 학습 시점에 넣으세요.
...
```text
가중치 조정 후 재학습 완료!
['#시험대비요약(Hist버전).ipynb', '#시험대비요약1(한가지 통합).ipynb', '#시험대비요약2(전략).ipynb', '1. 선형회귀분석_공유용.ipynb', '10.신용카드이상탐지.ipynb', '2. 다중회귀분석_공부시간시험성적_공유용.ipynb', '2. 앙상블_랜덤포레스트_과일종류맞추기_공유용.ipynb', '3. 군집분석_KMeans_공유용.ipynb', '4. 군집분석_DBSCAN_공유용.ipynb', '5. 로지스틱회귀분석_공유용.ipynb', '7. KNN_수박_참외_맞추기_공유용.ipynb', '8. K_Fold_교차검증_공유용.ipynb', '9. 그리드서치_공유용.ipynb', 'best_model.pkl', 'KNN_model.pkl', 'selected_features.pkl', 'submission.csv', '머신러닝실습용자료']
...
1. 대처법: pd.read_csv() 함수 안의 파일명을 확인하세요. 폴더 내 실제 파일명(예: exam_train.csv)과 철자, 대소문자까지 동일하게 수정해야 합니다.

### 1.KeyError

원인: 정답 컬럼명(TARGET)이 코드와 일치하지 않습니다.

1.. 대처법: print(df.columns)를 실행하여 데이터 프레임에 있는 실제 컬럼명을 확인하세요. 이후 코드 상단 TARGET = "..." 부분의 이름을 정확히 수정합니다.

### 2. ValueError (수치화 불가)

원인: 데이터 내에 숫자가 아닌 문자열이나 범주형 데이터가 포함되어 있습니다.

1. 대처법: Cell 1 하단에 X = X.select_dtypes(include=[np.number])를 추가하여 숫자형 데이터만 학습하도록 필터링하세요.

### 3. ImportError (모델 미지원)

원인: 시험장의 파이썬 환경이 낮아 HistGradientBoostingClassifier를 지원하지 않습니다.

1. 대처법: GradientBoostingClassifier로 즉시 변경하세요. 이때, 모델의 class_weight 옵션 대신 .fit() 메서드에 sample_weight=np.where(y == 1, 7, 1) 옵션을 추가해야 합니다.
...
경고(Warning) 대처: ConvergenceWarning이 발생한다면 반복 횟수가 부족한 것이므로 max_iter를 100에서 200으로 늘리거나 learning_rate를 낮추어 학습을 더 정교하게 만드세요.
